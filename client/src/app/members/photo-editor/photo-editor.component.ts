    ...
    this.memberService.setMainPhoto(photo.id).subscribe({
      next: () => {
        if (this.user && this.member) {
          ...
          this.accountService.setCurrentUser(this.user); // na ovaj nacin kad izmjenimo korisnika, sve komponente koje koriste korisnika ce da izmjene korisnika kod sebe
          this.member.photoUrl = photo.url;
          this.member.photos.forEach((x) => {
            if (x.isMain) x.isMain = false;
            if (x.id == photo.id) x.isMain = true;
          }); // da se odmah i na komponenti izmjeni stanje
        }
      },
    });
  }
